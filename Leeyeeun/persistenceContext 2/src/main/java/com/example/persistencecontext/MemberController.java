...
public class MemberController {
    ...
    public ResponseEntity<String> checkEntityIdentity(@PathVariable Long id) {
        ...
        return ResponseEntity.ok("동일성 확인!");
    }

    @PatchMapping("/merge/{id}")
    public ResponseEntity<Member> detachAndMerge(@PathVariable Long id) {
        return ResponseEntity.ok(memberService.detachAndMerge(id));
    }
}
